package cn.zhuoqin.platform.memcached;
...
public class MemcachedUtil {
	...
	public static boolean containsKey(String key) {
		boolean flag = false;
		try {
			flag = cachedClient.keyExists(key);
		} catch (Exception e) {
			// 记录Memcached日志
			MemcachedLog.writeLog("Memcached set方法报错，key值：" + key + "\r\n" + exceptionWrite(e));
		}
	...
	public static String[] splitKeys(String source, String delimiter) {
	...
	public static List getByPage(String key) {
		List result = null;
		// 根据拆分的KEY分别获取缓存内容
		String[] pageKeys = MemcachedUtil.splitKeys(MemcachedUtil.get(key).toString(), ",");
		if (pageKeys != null && pageKeys.length > 0) {
			result = new ArrayList();
			for (String pageKey : pageKeys) {
				result.addAll((List) MemcachedUtil.get(pageKey));
			}
		}
	...
	public static void cacheByPage(String key, List source, int cacheSize, Date expiredTime) {
		...
		if (source != null && source.size() > 0) {
			...
			int pageIndex = 0;
			while (true) {
				String itemKey = key + "_P_" + (pageIndex + 1);
				itemsKeys += itemKey + ",";
				List newList = new ArrayList();
				if ((pageIndex + 1) * cacheSize <= source.size()) {
					newList.addAll(source.subList(pageIndex * cacheSize, (pageIndex + 1) * cacheSize));
					set(itemKey, newList, expiredTime);
				} else {
					newList.addAll(source.subList(pageIndex * cacheSize, source.size()));
					set(itemKey, newList, expiredTime);
					break;
				}
			...
			itemsKeys = itemsKeys.substring(0, itemsKeys.length() - 1);
		}

		set(key, itemsKeys.trim(), expiredTime);
	}

	public static void removePage(String key) {
		String[] pageKeys = splitKeys(get(key).toString(), ",");
		if (pageKeys != null && pageKeys.length > 0) {
			for (String pageKey : pageKeys) {
				delete(pageKey);
			}
